...
**Virtualized Grid Scrolling** — A generic "scroll every large scrollable element to its bottom" sweep does not reliably render off-screen rows in virtualized grids/lists (Vaadin Grid, react-window, ag-grid, and similar) — they mount only visible rows and recycle DOM nodes as you scroll. Look for the component's own scroll-to API first (`grid.scrollToIndex(n)` for Vaadin Grid; analogous methods exist on other virtualization libraries) and call it directly via `page.evaluate()`. Verify the target content actually appears in `document.body.innerText` afterward — don't assume a scroll call worked just because it didn't throw. When you then need to click something inside a freshly-scrolled virtualized row, locate and click it in the **same** `page.evaluate()` call — locating in one script run and clicking in a later one risks the grid recycling that row (or its scroll position drifting) in between, hitting the wrong element. See **Anti-Patterns 10 and 11**.

**Common Anti-Patterns** — Most of these failure modes are now prevented by positive rules in **Key Rules** below (Selectors, Strict mode, Selector fallback, Actions vs reads, Visibility-not-presence). `anti-patterns.md` is the symptom-recognition reference for when a failure has already occurred — covers: text-based filter ambiguity, `page.evaluate` clicks not triggering React/Fluent UI synthetic events, diagnosing click failures from screenshots not error text, phantom `display:none` dialogs left in the DOM, role-vs-tag selector gaps (semantic HTML buttons that aren't `[role="button"]`), Fluent UI icon-button exact-match failures (use `:has(span.fui-Button__icon)` instead), role-less confirmation dialogs (use geometry-based modal detector when `[role="dialog"]` returns nothing), virtualized grids not rendering off-screen rows under generic scrolling, locate-then-click across separate script runs hitting a recycled row, screenshot-pixel vs. CSS-pixel coordinate mismatches when clicking by position, and framework tooltip components (e.g. `vaadin-tooltip`) that aren't discoverable via `title`/`aria-label` attribute scans.
