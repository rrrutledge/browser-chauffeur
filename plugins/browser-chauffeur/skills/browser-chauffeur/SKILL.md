...
**Automatic tab sweep.** When reusing an existing browser, this script keeps the tab count low. Reclaiming is driven by idleness; ownership just lets it clean up promptly. Three layers: (1) **owner reap** — a tab whose owning Claude session has ended is closed right away. Each tab is tied to the session that opened it, not the short-lived script (a session fires many scripts), so a tab stays alive as long as its session's window is open and is cleaned up when that window closes. (2) **Idle age-out** — any tab idle longer than the TTL (default 12h, override with `BROWSER_CHAUFFEUR_TAB_TTL`) is closed, regardless of owner, catching genuinely abandoned tabs. (3) **Count cap** — if more than `MAX_TABS` (default 10, override with `BROWSER_CHAUFFEUR_MAX_TABS`) remain, close the least-recently-active until back under it. Activity is tracked automatically — a tab created or reused via `openTab`/`findTab`, or one whose URL/title the sweep sees change between runs, counts as recently active, so an in-use tab is never the one closed. It never closes the browser's last page. Layers 2–3 also cover tabs opened without the `openTab` helper (which have no owner to reap). This keeps `connectOverCDP` fast and reliable and stops the browser accumulating enough tabs to crash (see **Resilient Connection**). It's automatic — you don't invoke it directly.

**Tying tab ownership to a session.** A tab's owner is read from two paired env vars: `BROWSER_CHAUFFEUR_OWNER_PID` — the long-lived process whose liveness the sweep checks — and `BROWSER_CHAUFFEUR_OWNER_START`, that process's creation time. The start time is what lets the sweep tell a session's tabs from those of a later process that inherits the same PID after the session ends; Windows recycles PIDs quickly, so without it the owner reap can never prove a recycle and an ended session's tabs survive until the idle-TTL or the count cap. There are two ways they get set:
- **Launched sessions** (drainer workers, handoffs via the session-mgr plugin's `launch-session.ps1`) get both automatically — the host tab's PID and its start time.
- **A session you start yourself** (open a terminal, type `claude`) sets them once from your shell. Add to your PowerShell `$PROFILE`:
  ```powershell
  $env:BROWSER_CHAUFFEUR_OWNER_PID = $PID
  $env:BROWSER_CHAUFFEUR_OWNER_START = (Get-Process -Id $PID).StartTime.ToFileTime()
  ```
  Process-scoped so each terminal owns the tabs opened from it (do **not** promote either to a persistent User env var, which would freeze one terminal's PID). Takes effect in the next new terminal.

When the PID var is unset, ownership falls back to the node script, so a tab is reclaimed shortly after the script that opened it exits — still correct, just not session-lifetime. When the PID is set but the start time isn't, the owner reap still reclaims a tab once its PID is fully free, but cannot reclaim one whose freed PID a live process has already reused — so set both.
...
2. **Backstop — self-heal, never hang.** `connectBrowser()` in `script-template.js` races `connectOverCDP` against a 30s hard timeout (`Promise.race`). On a wedge it reaps orphaned and over-the-cap tabs (`reapTabs`, which runs the same liveness-aware sweep as `chauffeur.py`) and retries the attach once, so the common wedge — an accumulation of tabs left open by ended sessions — clears on its own with no human step. Only a genuinely unrecoverable browser (a wedged renderer no reap can free) still surfaces, now with an actionable error. Every ad-hoc script you generate must use this hardened `connectBrowser()` — never call `chromium.connectOverCDP(...)` bare.

**If the connect still fails after that self-heal:** the overload is from tabs the reap won't touch — an active session's tabs or the user's own — or a wedged renderer. **Do not reset the shared browser to fix your own connect** — `cleanup-browser.py --reset` kills the browser for every concurrent session and wipes all logins. Instead, surface the situation to the user via `AskUserQuestion` and let them decide whether to close tabs or reset.
...
2. **Diagnose the cause** from what you see:
   ...
   - **connectOverCDP timeout** — output contains `connectOverCDP did not complete within ...ms` → `connectBrowser()` already reaped stale tabs and retried the attach once (see **Resilient Connection** above), and it still failed, so the overload is from tabs the reap won't touch (an active session's or the user's) or a wedged renderer — escalate via `AskUserQuestion`. Do NOT auto-reset the shared browser; that kills every concurrent session and wipes all logins.
