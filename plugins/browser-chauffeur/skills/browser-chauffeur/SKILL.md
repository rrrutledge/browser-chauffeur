...
**Automatic tab sweep.** When reusing an existing browser, this script keeps the tab count low in three layers: (1) it reclaims tabs whose **owning session has ended** — each tab is tied to the Claude session that opened it, not the short-lived script (a session fires many scripts), so a tab stays alive as long as its session's window is open and is reclaimed when that window closes; (2) it ages out any tab idle longer than the TTL (15 min, override with `BROWSER_CHAUFFEUR_TAB_TTL`); and (3) it enforces a hard ceiling of 10 open tabs (override with `BROWSER_CHAUFFEUR_MAX_TABS`), closing the least-recently-active first (idle/unowned tabs before any live session's tabs). Activity is tracked automatically — a tab created or reused via `openTab`/`findTab`, or one whose URL/title the sweep sees change between runs, is treated as recently active. It never closes a tab a live session still owns (except as a last resort at the hard ceiling), and never the browser's last page. Layers 2–3 are the backstop that catches tabs opened without the `openTab` helper — those aren't registered, so layer 1 can't see them. This keeps `connectOverCDP` fast and reliable and stops the browser accumulating enough tabs to crash (see **Resilient Connection**). It is automatic and safe — you don't invoke it directly.

**Tying tab ownership to a session.** A tab's owner is read from two env vars: `BROWSER_CHAUFFEUR_OWNER_PID` (the long-lived process whose liveness the sweep checks) and `BROWSER_CHAUFFEUR_OWNER_SESSION` (the Claude session id, for traceability). Sessions launched via `scripts/launch-session.ps1` (drainer workers, handoffs) get these set automatically to the host tab's PID + `--session-id`. For a Claude session you start **interactively** from a shell, set the owner once so its tabs are owned by that terminal rather than the ephemeral node script — e.g. add to your PowerShell `$PROFILE`: `$env:BROWSER_CHAUFFEUR_OWNER_PID = $PID`. When neither var is set, ownership falls back to the node script (tabs are reclaimed shortly after each script exits) — still correct, just not session-lifetime.
...
1. **Root cause — keep the tab count low.** Scripts create every tab with `openTab` (from `browser-chauffeur-helpers`), which registers it against the owning Claude session, and `closeTab` unregisters on clean close; `launch-browser.py` reclaims tabs whose owning session has ended on every reuse. A tab opened without `openTab` is never registered, so it isn't tied to a session — which is why `openTab` is the only sanctioned way to open a tab (see Phase 1). As a backstop for any tab that still slips through, the same sweep ages out idle tabs past the TTL and enforces a hard ceiling on total tabs (evicting the least-recently-active, idle/unowned before any live session's tabs). So a healthy profile stays small and `connectOverCDP` stays fast — this is what makes it reliable, not just recoverable.
