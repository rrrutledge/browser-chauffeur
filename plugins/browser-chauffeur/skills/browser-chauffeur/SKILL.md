...
**Tying tab ownership to a session.** A tab's owner is read from two env vars: `BROWSER_CHAUFFEUR_OWNER_PID` (the long-lived process whose liveness the sweep checks) and `BROWSER_CHAUFFEUR_OWNER_SESSION` (the Claude session id, for traceability). There are two ways they get set:
- **Launched sessions** (drainer workers, handoffs via `scripts/launch-session.ps1`) get them automatically — the host tab's PID + the `--session-id`.
