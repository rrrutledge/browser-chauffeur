...
Edit a Google Doc's content directly through the Docs API's `batchUpdate` endpoint: `scripts/google-docs.js`.
...
2. **Install the dependency** - `node scripts/setup.js` installs `google-auth-library` into `~/.claude/google-docs/node_modules`.
