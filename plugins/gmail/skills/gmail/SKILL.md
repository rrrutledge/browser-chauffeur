...
- **`gmail.js`**
  ...
  - **Body file is HTML, always.** Both `--reply` and `--draft-new` insert the `--body-file` contents into
    an HTML message body verbatim, so bare line breaks collapse — paragraphs must be `<p>…</p>` (or `<br>`),
    and links must be `<a href="URL">anchor text</a>` (never a bare URL pasted in prose, per document-authoring).
    Write the body file as HTML; a plain-text `.txt` renders as one run-on block.
