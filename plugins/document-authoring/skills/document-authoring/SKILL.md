...
- **A repeat nudge on an already-unanswered ask shrinks further than the first** — drop the warm-up context line and any separate reassurance clause, and ask one direct binary question with the release folded into the question itself rather than tacked on after.
  Keep the short restated ask ("wanted to ask again about this") even when the unanswered message sits visibly elsewhere in the same thread, and pair it with the directional pointing emoji — 👆 for a Slack/Teams thread, where the earlier message sits above; 👇 for an email reply, where the quoted original sits below the new text. This pointer commonly stacks with 🙏 (and optionally ❓) past the one-emoji cap and the palette — "wanted to ask again about this 👇🙏❓" — see the Emoji section's stacking exceptions.
  **When the repeat carries genuine new content** (a lower-commitment alternative, fresh information), write the restated ask out alongside the new material the same way, rather than leaning on the pointing emoji alone.
...
A fourth exception is a repeat nudge's directional pointer (👇/👆, see the palette above) stacking with 🙏 and optionally the off-palette ❓ — "wanted to ask again about this 👇🙏❓" — past both the one-emoji cap and the palette, the same way the gratitude stack does.
