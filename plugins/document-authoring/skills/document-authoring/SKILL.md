...
- **Adding a new recipient to an existing thread: state the action, don't address them.**
  ...
  **When the plan to add them is itself a reply to the correspondent's pushback, confirm with the correspondent before cc'ing the new person in** - close with a short open check ("Does that sound OK?") instead, and loop the new person in on a later message once the correspondent has actually agreed.
