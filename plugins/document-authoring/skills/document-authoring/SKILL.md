...
- **Keep asks open and tentative — don't pre-commit.**
  ...
  A related secondary consideration tacked onto the primary ask compresses into a brief hypothetical question ("and if X goes live, we can include that?") rather than a full second request with its own imperative ("could you send me X") and explicit release clause - let the recipient infer what to do with it instead of spelling out the mechanism.
