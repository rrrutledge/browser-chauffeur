...
- **When asking someone to make an introduction, just ask — don't supply a script.** Make the single question ("Would you be comfortable introducing us?") and stop. Providing a pre-written blurb for the introducer adds overhead and presumes how they should phrase it. Real edit (asking Yuki to intro Russell to Tomohiro Nakajima at KDDI): a draft that followed the question with "If so, here's a short blurb you can use: [blurb]  Happy to adjust anything if you'd like to word it differently." was sent as just the question alone.
