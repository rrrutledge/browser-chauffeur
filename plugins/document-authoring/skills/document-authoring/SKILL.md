...
A fourth exception is a light-touch, personal nudge in a congregation-membership-clerk register (a repeat ask sent as someone's local record-keeper, not an institutional or work message), where Russell stacks 🙏 and ❓ past both the one-emoji cap and the documented palette ("wanted to ask again if you can let me know about this 🙏❓"); the pointing emoji (👇/👆) is optional on top of that stack, not required - Russell includes it on some sends of this exact nudge and drops it on others, since "let me know about this" already names the referent.
