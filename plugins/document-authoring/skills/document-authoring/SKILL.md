...
- **When a decision-relevant fact is still outstanding, get it before committing — and don't let the wait look like stalling.**
  ...
  **The same holds for accepting an offer of someone's time or help** — before agreeing to add a person to a piece of work, ask what the support would actually involve, rather than a bare "sounds good"; a real add competes against other commitments (a concurrent project, an event) and that tradeoff is itself the fact still outstanding.
