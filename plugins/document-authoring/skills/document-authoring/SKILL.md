...
- **When a friend vents and asks something, engage in short curious back-and-forth — don't package up sympathy and an offer.** Answer the question in one brief hedged line, then get curious about the specifics with a follow-up question or two, each as its own one-line message. Skip the composed empathy paragraph ("you deserve a lot better than that") and skip any unsolicited offer of help — a real conversation draws the person out, it doesn't close with a statement. Real edit (a DM where a friend said he'd lost confidence in his job after a bad rating and asked if WellSky hires remote): a single staged reply — "Hey Jeff - that's rough.  Lowering your rating with zero context and then not even showing up to explain it isn't right, and you deserve a lot better than that. 🙁  On WellSky - it's still very much on-site here in Kansas City, so a remote role isn't really in the cards. But if you'd ever consider a move to KC, I'd be glad to help you look into it." — was sent instead as three short messages: "Yeah - I'm pretty sure it's still KC-focused." / "\"former\" boss?" / "So you got rated below the \"consistent\" rating (whatever the middle/normal rating is)?" The empathy and relocation offer were cut entirely, the answer hedged ("pretty sure" not "for sure"), and the reply turned into curiosity.
