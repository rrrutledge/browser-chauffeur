...
- **Keep asks open and tentative — don't pre-commit.**
  ...
  **When nudging someone to make an introduction for a delicate ask (money, sponsorship) where a misjudged approach risks offending the contact, and the intermediary knows that contact better than you do, ask sincerely whether they still think it's a good idea** ("if you think that's a good idea?") instead of a generic bandwidth release ("no worries if you're swamped") - this is a real question deferring to their read of the contact, not a hedge about their being busy.
