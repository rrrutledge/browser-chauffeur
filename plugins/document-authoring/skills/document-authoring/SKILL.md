...
3. For each voice change, fold it into the matching section — **Formal writing** if the message was formal, **Conversational writing** if it was a chat/email reply. **Default to enriching an existing bullet** — scan for a bullet whose subject overlaps the new lesson and add the new real example to it. Only add a new bullet when no existing one covers the subject at all. State it positively. Don't accumulate near-duplicates.
