...
- **Keep asks open and tentative — don't pre-commit.**
  ...
  **Nudging on a stalled favor (an intro, an ask relayed to someone else): release on the merit, not on their bandwidth.** Default drafting reaches for "no worries if you're swamped" - but when someone agreed to do something and then went quiet, the stall is as likely to be second thoughts about the ask itself as it is being busy. Release with "(if you think that's a good idea?)" instead, handing back whether they still endorse it, not just whether they have time for it.
