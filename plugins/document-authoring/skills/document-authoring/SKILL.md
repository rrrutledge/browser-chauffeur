...
- **Email greeting**: run it into the first sentence with ` - ` — "Hi [Name] - thanks and totally makes sense.", not "Hi [Name]," on its own line followed by a new paragraph.
  ...
  **When one recipient has something specific to react to — a question they asked, or a detail they shared (their own news, a plan they mentioned) — acknowledge them by name and skip the greeting line entirely, then address any other recipient by name in turn.** "Thanks for considering it, Pooi." carries both the warm open and the address, and the group greeting would aim the reply at people who are only watching. This holds even when the other recipient is a brand-new introduction being met for the first time on the same thread — greet each person by name as the message reaches them, rather than opening with a plural "Hi both" that speaks to no one in particular.
