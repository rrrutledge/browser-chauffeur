...
- **Keep asks open and tentative — don't pre-commit.**
  ...
  When asking an existing contributor for more, let a peer already committed carry the invitation instead of pairing it with a separate stated desire ("we'd love to have you back") - name the peer, then hand the choice back with an indirect close ("wanted to share it with you in case you'd like to as well") or an explicit release ("so it's fine either way").
