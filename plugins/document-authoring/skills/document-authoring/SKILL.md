...
- **Energizing close** — how to execute a warm close, when **Register** calls for one: end with a brief forward-looking note ("Looking forward to it!", "This'll be great") instead of a helper-offer tail.
  ...
  Skip the note when sharing news into someone else's channel or group, when the message already states a concrete, dated next step (a call, a follow-up date), when the message itself already commits to a self-initiated follow-through action (the "I'll take a look and let you know" pattern below - that commitment is already the forward motion, so a separate "Looking forward to it" on top of it is redundant), or when the note would restate something the recipient hasn't agreed to yet - see the Asks "when a contact declines" rule; a plain thanks or "no worries" still closes the message fine.
