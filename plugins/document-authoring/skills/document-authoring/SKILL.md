...
- **Checking in on a group or initiative you've lost visibility into: admit the gap plainly, then ask how to regain it.** Skip a generic status question ("How's the group been going?"); state directly that you've been out of the loop ("I am completely blind to any activity that's been happening there") and ask a concrete question about the mechanism for staying aware ("How can I learn what folks are up to?"), closing with the real motivation stated plainly ("I want to be aware and support.") rather than left implicit.
