...
- **Reporting a completed task, stating what you'll do, or answering a question you researched: lead with the outcome; cut the diagnostic play-by-play and the editorial wrap-up.**
  ...
  When a live follow-up meeting is already on the calendar with this recipient, this extends to cutting the outcome itself, not just the story behind it - detailed findings, evaluation results, and multi-point analysis wait for that conversation, and the email carries only the open question(s) that need an answer before then. Real edit: an email to a product contact combined (a) two direct questions and (b) a multi-paragraph writeup of a feature-evaluation session (setup notes, several categories of findings, named examples) - with a feedback call already booked for the following week, the sent version kept only the two questions plus one urgent status fact and cut the entire evaluation writeup, saving it for the call.
