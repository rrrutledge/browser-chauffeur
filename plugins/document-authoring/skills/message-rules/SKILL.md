...
- **Exception - the other side already responded, apologetically, with a promise to follow up** (a declined meeting, "I'll get back to you") **rather than staying silent:** close on a low-pressure wish instead of a "let me know" ask ("Hope that it works out."), since they've already explained themselves and an active ask reads as more demanding than the situation calls for.
  This exception persists on every later touch, not just the first — a second or third nudge on the same still-unanswered promise stays with the low-pressure wish rather than shrinking into the repeat-nudge bullet's binary question above, since the contact explained themselves once and never went silent to begin with.
