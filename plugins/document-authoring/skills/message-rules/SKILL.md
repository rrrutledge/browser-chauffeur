...
- **Phrase a request or open decision as a question, not a flat directive or assertion, even when you already know the answer.** For a requested edit to someone else's draft, "can you make it X?" reads as collaborative where the flat "should be X" reads as correction; more generally, posing a decision as an open question — even a confident one ("Do we need X, or does Y happen automatically?") — hands the other person ownership of it and keeps them motivated to follow through, where asserting it outright reads as being told what to do. Save the flat framing for reference material, not a live ask.
...
- **When a cause or mechanism isn't actually confirmed, ask rather than assert it as fact.** This covers diagnosing someone else's reported problem (a screenshot or description is evidence, not proof — a genuine "How did you get to that page?" finds the real cause and reads as curious, not defensive) as much as your own guess at how a process or system works (e.g. whether a step runs once or on every update) — a stated conclusion that turns out wrong ("that page is misconfigured") commits you to it in writing, so ask the open question instead of asserting it.
