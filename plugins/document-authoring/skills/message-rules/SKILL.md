...
- **Energizing close** — how to execute a warm close, when **Register** calls for one: end with a brief forward-looking note ("Looking forward to it!", "This'll be great") instead of a helper-offer tail.
  ...
  Skip the note when sharing news into someone else's channel or group, when the message already states a concrete, dated next step (a call, a follow-up date), when the note would restate something the recipient hasn't agreed to yet - see the Asks "when a contact declines" rule, or when its content would evaluate your own fit or qualifications for something (e.g. a role a referral just came through for) rather than the relationship or shared moment - that reads as self-promotion, not warmth toward the person who just helped; a plain thanks or "no worries" still closes the message fine.
