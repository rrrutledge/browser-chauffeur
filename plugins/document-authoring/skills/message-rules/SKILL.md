...
- **Frame a proactive, unsolicited update - something new you're sending because you thought of the person, not the check-in-with-nothing-to-report just above - as a no-obligation share:** open on genuine personal warmth ("I was thinking of you today"), and close by naming it as just an update with a plain well-wish ("Anyway, thought I'd send the update. Hope you're doing well.") that asks nothing back, rather than pitching the update's usefulness up front or ending on a question that expects a reply.
  **Compiling this kind of update from content written for other correspondents (older sent mail, notes to a different friend): keep only what's genuinely relevant to this recipient, and re-screen anything that reveals real vulnerability (financial strain, a hard job search) for this specific audience even when it already appeared elsewhere.** Favor a detail that mirrors what this recipient is themselves currently doing over a generic roundup of unrelated personal news gathered from elsewhere.
