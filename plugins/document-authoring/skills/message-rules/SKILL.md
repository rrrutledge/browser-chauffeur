...
- **Email sign-off**: just `Russ` on its own line — no valediction before it, and no closing word fused onto that same line (no "Thanks, Russ", "Best, Russ", or similar; the line is `Russ` alone, full stop). A genuine thanks belongs in the body as its own sentence (see **Light, genuine appreciation**), not welded onto the sign-off — and skip it entirely when there's nothing yet to thank the recipient for, e.g. a first-touch ask where they haven't done anything for you yet.
  ...
  **A reply-by-email that actually posts as a comment on a public thread (a GitHub issue or PR, where replying to the notification email adds a comment there) drops both the greeting and the sign-off, not just the sign-off.** The platform attributes the comment to him and threads it under the existing discussion on its own, so the composed text is only the comment's content — no "Hi [Name]" opener, no `Russ` line, the same reasoning as the auto-appended signature above, applied to the platform's own attribution instead.
