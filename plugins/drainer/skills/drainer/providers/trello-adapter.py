...
or have no due date, ranked most-recently-due first. An undated card has no deadline, so it's ranked by
its creation date (decoded from the card's ObjectId) rather than pinned to the top of the queue.
Credentials are TRELLO_API_KEY / TRELLO_TOKEN in the environment (read by trello_utils.get_trello_session).
"""
import glob
import json
import os
...
class Provider(ProviderBase):
    ...
    def _created_dt(card_id):
        ...
        ranks by age instead of being pinned to the top of the queue."""
        try:
    ...
    def _enumerate(self, limit):
        ...
        for board in self.boards:
            ...
            for card in self._utils.get_board_cards(bid, self.session):
                ...
                if due_dt is not None and due_dt > now:
                    continue
                # Sort rank: a dated card ranks by its due date; an undated card has no deadline, so it
                # ranks by its creation date (always in the past) instead of being pinned to the top.
        ...
        # Most-recently-due first, where an undated card sorts by its creation date (set above). This is
        # also the truncation order: when more than `limit` cards are in play the lowest-ranked (oldest)
        # ones are dropped, and they resurface on a later cycle. A card whose sort date couldn't be
        # derived falls back to `now` so it's never silently buried.
