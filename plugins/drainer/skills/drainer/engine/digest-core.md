...
Each item carries a `disposition` its worker stamped (`abandoned` | `advanced` | `nudged`) and a one-line
`dispositionReason`. Present one distinct **"Auto-handled"** section (separate from fyi), split by that
disposition:
- **State changed (worth a glance)** - every item whose disposition is anything other than `nudged`:
  `abandoned` (terminal - a role died, a thread ended), `advanced` (moved a stage, or a standing action
  ran), and any item that carries no disposition at all. One line each, printed with its
...
- **Checked, no change** - the `nudged` items (a cadence bump on an unanswered card, an already-handled
  no-op). Collapse these to a single count with a short by-source breakdown ("Checked, no change: 9
  outreach cards bumped"), not a line each. Surface an individual nudge only when its reason flags
  something odd.
