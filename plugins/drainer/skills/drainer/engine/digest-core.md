...
matters. **Draw that substance from the captured body, not from the subject.** Many notifications carry
a subject that only announces *that* something happened - a platform relaying that someone posted in a
group, commented on a thread, or sent a message - while the real content sits in the body the provider
captured (an email item's `emailFile`, a Slack item's `bodyFile`). Read that body and summarize what it
actually says, the same way you'd summarize a newsletter's body, never the announcement line restated.
For a content-carrying bulletin - a school/HOA/church newsletter, a class welcome, or any
"someone posted"-style bulletin whose substance is in the body - also pull the to-dos out of the body
into a short **action items for you** list, the way a meeting recap surfaces its owner-assigned next
steps: every form to sign and return, supply list, deadline, RSVP, or link he's meant to open. Link the
source with descriptive text per the `document-authoring` voice, never a bare URL. Order by what's most
worth knowing first.
