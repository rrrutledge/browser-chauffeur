...
the proposal from it — don't hand-write a rule from memory. Run the proposed phrase through that skill's
reviewer pass (its **The reviewer** section, a cold `mail-filter-review` check) before proposing it - a
company-specific `from:<sender>` filter, or a brand token left in the phrase, is the tell that the skill
or its reviewer was skipped.
