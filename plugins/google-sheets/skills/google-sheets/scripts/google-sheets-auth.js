// One-time interactive sign-in for the Google Sheets API (spreadsheets scope). Runs the OAuth2
// authorization-code loopback flow: prints (and serves) a consent URL, captures the code on a local
// callback, exchanges it for tokens, and caches them to ~/.claude/google-sheets/oauth-token.json.
//
...
//
//   node google-sheets-auth.js   -> start the flow on http://localhost:8712/callback

const http = require('http');
const { buildOAuthClient, writeTokens, SCOPES, REDIRECT_URI } = require('./google-sheets-oauth');
...
(async () => {
  ...
  writeTokens(tokens);
  console.log('Signed in — Sheets API token cached. google-sheets.js will run silently now.');
